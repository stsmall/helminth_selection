...
def freqVsel_stats(freqtrace, time, Ne, gens):
    """
...
def freqVsel_sims(msms, Ne, pops, reps, s, rho, theta, sp, smu, sAAc, sAac,
                  saac, sft, sff, gens, selp, time, threads):
    """
    Simulation in msms, records the trace and searches for the derived allele
    frequency at stated time in past dependent on the selection strength
    """
    freq = []
    nhap = pops[0]
    orig = []
    for selco in selp:
        ...
        # calc stats
        freq.extend(freqVsel_stats(freqtrace, time, Ne, gens))
        orig.append(np.repeat(sum([i > 1 for i in origcount])/float(reps),
                              len(time)))
    dfFreqVsel = pd.DataFrame({'SAA': np.repeat(selp * sAAc, len(time)),
                               'SAa': np.repeat(selp * sAac, len(time)),
                               'time': time * len(selp),
                               'freq': freq,
                               'orig': np.concatenate(orig).ravel()
                               })
    dfFreqVsel = dfFreqVsel.loc[:, ['SAA', 'SAa', 'time', 'freq', 'orig']]
    dfFreqVsel.to_csv("FreqvarySel_helminth.csv")
    return(dfFreqVsel)
...
def hapmigVsel_sims(msms, Ne, pops, reps, s, rho, theta, sp, smu, sAAc, sAac,
    ...
    for selco in selp:
        for m in migp:
            ...
            gtdict, posdict, origcount, freqtrace = parse_msfile(msout, nhap,
                                                                 reps)
            # calc stats
            stats_out = hapmigVsel_stats(gtdict, posdict, pops, sp, origcount,
                                         selco, m)
            Piplot = stats_out[0]
            freqR = stats_out[1]
            ra = stats_out[2]
...
if __name__ == '__main__':
    ...
    if args.optionA:
        freqVsel_sims(msms, Ne, pops, reps, s, rho, theta, sp, smu, sAAc,
                      sAac, saa, sft, sff, gens, selpA, time, threads)
    if args.optionB:
        hapmigVsel_sims(msms, Ne, pops, reps, s, rho, theta, sp, smu, sAAc,
                        sAac, saa, sit, sif_l, selpB, migp, threads,
                        join_times, gens)
